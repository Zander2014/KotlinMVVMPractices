package pers.zander.utils.ktx.core.func

import androidx.annotation.NonNull
import pers.zander.utils.ktx.ext.canListFiles
import java.io.*
...
fun getFolderSize(file: File): Long {
    ...
    //for (subFile in file.listFiles())//此写法会出现NullPointException
...
fun getAllSubFile(folder: File): Array<File> {
    ...
    //for (subFile in folder.listFiles())//此写法会出现NullPointException
...
fun copyFolder(sourceFolder: File, destFolder: File, overwrite: Boolean, func: ((file: File, i: Int) -> Unit)? = null) {
    ...
    //for (subFile in sourceFolder.listFiles()) {//此写法会出现NullPointException
